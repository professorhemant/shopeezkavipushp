    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-0.5">Top performers and business insights</p>
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-6">
          ...
          <div className="bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden">
            ...
            {topProducts.length === 0 ? (
              <div className="flex items-center justify-center py-16 text-slate-400"><p className="text-sm">No data available</p></div>
            ) : (
                  ...
                  return (
                    <div key={p.id || i} className="px-4 py-3">
                      <div className="flex items-center justify-between mb-1.5">
                        <span className="text-sm font-medium text-slate-800 truncate max-w-[60%]">{p.name || p.product_name}</span>
                        <div className="text-right">
                          <p className="text-sm font-semibold text-slate-800">{formatCurrency(p.total_sales || p.revenue || 0)}</p>
                          <p className="text-xs text-slate-400">{value} units</p>
                        </div>
                      </div>
                      <div className="h-1.5 bg-gray-100 rounded-full overflow-hidden">
                        <div className="h-full bg-amber-500 rounded-full" style={{ width: `${(value / max) * 100}%` }} />
                      </div>
                    </div>
                  )
                })}
          ...
          <div className="bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden">
            <div className="px-4 py-3 border-b border-slate-100 flex items-center gap-2">
              ...
              <h2 className="text-base font-semibold text-slate-800">Top Customers</h2>
            </div>
                ...
                {topCustomers.map((c, i) => {
                  ...
                  return (
                    <div key={c.id || i} className="px-4 py-3">
                      <div className="flex items-center justify-between mb-1.5">
                        <span className="text-sm font-medium text-slate-800 truncate max-w-[60%]">{c.name || c.customer_name}</span>
                        <div className="text-right">
                          <p className="text-sm font-semibold text-slate-800">{formatCurrency(value)}</p>
                          <p className="text-xs text-slate-400">{c.invoice_count || c.orders || 0} orders</p>
                        </div>
                      </div>
                      <div className="h-1.5 bg-gray-100 rounded-full overflow-hidden">
