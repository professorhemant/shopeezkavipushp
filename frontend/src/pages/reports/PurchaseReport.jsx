    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">Purchase analysis for the selected period</p>
        </div>
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          { label: 'Total Bills', value: summary.count, color: 'text-gray-900' },
          { label: 'Total Amount', value: formatCurrency(summary.total), color: 'text-blue-600' },
              ...
              <tbody>
                {data.map((row, i) => (
                  <tr key={i} className="border-b hover:bg-gray-50">
                    <td className="px-4 py-3 text-gray-900 font-medium">{row.period || row.supplier_name || row.date}</td>
                    <td className="px-4 py-3 text-right text-gray-600">{row.count || row.bills || 0}</td>
                    <td className="px-4 py-3 text-right font-semibold text-gray-900">{formatCurrency(row.total || row.amount)}</td>
