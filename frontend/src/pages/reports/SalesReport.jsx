    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-0.5">Sales analysis for the selected period</p>
        </div>
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          { label: 'Total Invoices', value: summary.count, color: 'text-slate-800' },
          { label: 'Total Revenue', value: formatCurrency(summary.total), color: 'text-amber-600' },
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {data.map((row, i) => (
                  <tr key={i} className="border-b hover:bg-slate-50">
                    <td className="px-4 py-3 text-slate-800 font-medium">{row.period || row.date || formatDate(row.date)}</td>
                    <td className="px-4 py-3 text-right text-slate-600">{row.count || row.invoices || 0}</td>
                    <td className="px-4 py-3 text-right font-semibold text-slate-800">{formatCurrency(row.total || row.revenue)}</td>
                    <td className="px-4 py-3 text-right text-slate-600">{formatCurrency(row.taxable_amount || row.taxable)}</td>
              ...
              <tfoot className="bg-slate-50 font-semibold">
                <tr>
                  ...
                  <td className="px-4 py-3 text-right text-slate-800">{summary.count}</td>
                  <td className="px-4 py-3 text-right text-slate-800">{formatCurrency(summary.total)}</td>
                  <td className="px-4 py-3 text-right text-slate-800">{formatCurrency(summary.total - summary.tax)}</td>
                  <td className="px-4 py-3 text-right text-slate-800">{formatCurrency(summary.tax)}</td>
