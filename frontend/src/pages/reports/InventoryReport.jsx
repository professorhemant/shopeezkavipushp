    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">Current stock status and valuation</p>
        </div>
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          { label: 'Total Products', value: summary.total_items, color: 'text-gray-900' },
          { label: 'Inventory Value', value: formatCurrency(summary.total_value), color: 'text-blue-600' },
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 text-gray-400">
            ...
            <p className="text-base font-medium text-gray-500">No inventory data found</p>
          </div>
        ) : (
                ...
                {filtered.map((item) => (
                  <tr key={item.id} className="border-b hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-right">
                      <span className={`font-semibold ${item.is_low_stock || item.current_stock <= (item.min_stock || 0) ? 'text-red-600' : 'text-gray-900'}`}>
                        {item.current_stock ?? item.stock_qty ?? 0}
                      </span>
                    </td>
                    <td className="px-4 py-3 text-right text-gray-600">{item.min_stock || 0}</td>
                    <td className="px-4 py-3 text-right text-gray-600">{formatCurrency(item.purchase_price || item.unit_cost)}</td>
                    <td className="px-4 py-3 text-right font-semibold text-gray-900">{formatCurrency(item.stock_value || 0)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
