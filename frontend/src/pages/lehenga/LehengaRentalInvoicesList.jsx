...
export default function LehengaRentalInvoicesList() {
  ...
  const load = async () => {
    setLoading(true)
    try { const { data } = await lehengaAPI.listRentalInvoices(); setRows(data.data || []) }
  ...
  const remove = async (r) => {
    ...
    try { await lehengaAPI.deleteRentalInvoice(r.id); toast.success('Deleted'); load() }
    catch { toast.error('Failed to delete') }
  }

  const term = q.trim().toLowerCase()
  const visible = term
    ? rows.filter(r => [r.invoice_no, r.customer_name, r.code, r.name].some(v => (v || '').toLowerCase().includes(term)))
    : rows
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-wrap items-start justify-between gap-2">
        <div>
          <h1 className="text-xl sm:text-2xl font-bold text-slate-800">Lehenga Rental Invoices</h1>
          <p className="text-sm text-slate-500 mt-0.5">{rows.length} saved invoices</p>
        </div>
        <Link to="/lehenga/rental-invoice" className="bg-amber-600 hover:bg-amber-700 text-white px-4 py-2 rounded-lg text-sm font-medium flex items-center gap-2">
                ...
                {visible.length === 0 ? (
                  ...
                  return (
                    <tr key={r.id} className="border-b hover:bg-slate-50">
                      ...
                      <td className="px-4 py-3 text-slate-600">{r.code || r.name || '—'}</td>
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center justify-center gap-1">
                          <Link to={`/lehenga/rental-invoice/view/${r.id}`} className="p-1.5 rounded hover:bg-amber-50 text-slate-400 hover:text-amber-600" title="View / Print"><Eye className="h-4 w-4" /></Link>
