...
import BridalInvoiceDocument from './BridalInvoiceDocument'

export default function BridalInvoiceView() {
  const { id } = useParams()
  ...
  useEffect(() => {
    (async () => {
      try { const { data } = await bridalAPI.getInvoice(id); setInv(data.data || null) }
    ...
    <div className="space-y-5">
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-2 sm:p-4">
          <BridalInvoiceDocument ref={printRef} inv={inv} firm={firm} />
        </div>
      )}
    </div>
  )
