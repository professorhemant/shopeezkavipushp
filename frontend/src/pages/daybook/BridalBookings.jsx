...
export default function BridalBookings() {
  ...
  // Standalone checker debounce — requires both set code AND function date
  useEffect(() => {
    ...
    checkerDebounceRef.current = setTimeout(async () => {
      ...
      try {
        const res = await dayBookAPI.checkBridalAvailability(code, checkerDate)
      ...
      <div className="bg-white rounded-xl p-4 shadow-sm border border-slate-200">
        ...
        <div className="flex flex-wrap items-end gap-3">
          <div>
            ...
            <input
              value={checkerCode}
              onChange={(e) => { setCheckerCode(e.target.value); setCheckerResult(null) }}
              placeholder="e.g. A001"
              className="border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-amber-500/30 focus:border-amber-500 w-44"
            />
          </div>
          <div>
            <label className="block text-xs font-medium text-slate-600 mb-1">Function Date Required</label>
            <input
              type="date"
              value={checkerDate}
              onChange={(e) => { setCheckerDate(e.target.value); setCheckerResult(null) }}
              className="border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-amber-500/30 focus:border-amber-500 w-44"
            />
          </div>
          ...
          <div className="pb-0.5">
            {(!checkerCode.trim() || !checkerDate) && (
              <p className="text-xs text-slate-400 italic">Enter set code + function date to check</p>
            )}
            ...
            {!checkerLoading && checkerResult !== null && checkerCode.trim() && checkerDate && (
              checkerResult.length === 0 ? (
                <span className="flex items-center gap-1.5 text-green-600 text-sm font-semibold">
                  ...
                  &ldquo;{checkerCode}&rdquo; is available for {fmtDate(checkerDate)}
                </span>
              ) : (
                <div className="flex flex-wrap items-center gap-2">
                  <span className="flex items-center gap-1.5 text-red-600 text-sm font-semibold">
                    <AlertCircle className="h-4 w-4 shrink-0" />
                    &ldquo;{checkerCode}&rdquo; not available for {fmtDate(checkerDate)}:
                  </span>
