...
export default function SecurityRefund() {
  ...
  const load = async () => {
    setLoading(true)
    try { const { data } = await dayBookAPI.getSecurityRefunds(date); setRows(data.data || []) }
  ...
  const handleSubmit = async (e) => {
    ...
    setSaving(true)
    try {
      if (editId) { await dayBookAPI.updateSecurityRefund(editId, { ...form, date }); toast.success('Updated') }
      else { await dayBookAPI.createSecurityRefund({ ...form, date }); toast.success('Added') }
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-wrap items-start justify-between gap-2">
        <div>
          <h1 className="text-xl sm:text-2xl font-bold text-slate-800">Day Book — Security Refund</h1>
