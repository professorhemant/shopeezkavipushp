...
export default function BridalDispatch() {
  ...
  const handleSubmit = async (e) => {
    ...
    setSaving(true)
    try {
      if (editId) { await dayBookAPI.updateBridalDispatch(editId, { ...form, date }); toast.success('Updated') }
      else { await dayBookAPI.createBridalDispatch({ ...form, date }); toast.success('Added') }
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-slate-800">Day Book — Bridal Dispatch</h1>
