...
export default function BalanceSheet() {
  ...
  const Section = ({ title, items = [], total, colorClass = 'text-slate-800' }) => (
    <div>
      ...
      <div className="space-y-1">
        {items.map((item, i) => (
          <div key={i} className="flex items-center justify-between py-1.5 border-b border-slate-100">
            <span className="text-sm text-slate-600 pl-2">{item.label || item.name}</span>
            <span className="text-sm text-slate-800">{formatCurrency(item.value || item.amount || 0)}</span>
          </div>
        ))}
      ...
      {loading ? (
        ...
        <div className="grid md:grid-cols-2 gap-6">
          ...
          <div className="bg-white rounded-xl shadow-sm border border-slate-100 p-6 space-y-6">
            <h2 className="text-base font-semibold text-slate-800 border-b pb-2">Assets</h2>
            <Section
            ...
            <Section
              ...
              colorClass="text-amber-600"
            />
            <div className="flex items-center justify-between py-2 border-t-2 border-slate-200 font-bold">
              <span className="text-sm">Total Assets</span>
              <span className="text-sm text-amber-600">{formatCurrency(data?.total_assets || 0)}</span>
            </div>
          </div>

          {/* Liabilities & Equity */}
          <div className="bg-white rounded-xl shadow-sm border border-slate-100 p-6 space-y-6">
            <h2 className="text-base font-semibold text-slate-800 border-b pb-2">Liabilities & Equity</h2>
            <Section
            ...
            <div className="flex items-center justify-between py-2 border-t-2 border-slate-200 font-bold">
              ...
              <span className="text-sm text-amber-600">{formatCurrency(data?.total_liabilities_equity || 0)}</span>
            </div>
          </div>
        </div>
      )}
