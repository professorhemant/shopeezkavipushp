  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <p className="text-sm text-gray-500 mt-0.5">Outstanding amounts to suppliers</p>
      </div>

      <div className="grid grid-cols-2 gap-4">
        {[
          { label: 'Total Payable', value: formatCurrency(summary.total), color: 'text-red-600' },
          { label: 'Suppliers', value: summary.count, color: 'text-gray-900' },
        ].map((s) => (
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 text-gray-400">
            <TrendingDown className="h-12 w-12 mb-3 text-gray-300" />
            <p className="text-base font-medium text-gray-500">No outstanding payables</p>
          </div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {filtered.map((r) => (
                  <tr key={r.supplier_id || r.id} className="border-b hover:bg-gray-50 cursor-pointer" onClick={() => navigate(`/suppliers/${r.supplier_id || r.id}`)}>
                    <td className="px-4 py-3 font-medium text-blue-600">{r.supplier_name}</td>
                    <td className="px-4 py-3 text-gray-600">{r.phone || '-'}</td>
                    <td className="px-4 py-3 text-right text-gray-900">{formatCurrency(r.total_purchases || r.total)}</td>
