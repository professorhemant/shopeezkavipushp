...
export default function AddProductManual() {
  ...
  useEffect(() => {
    Promise.all([categoryAPI.getAll(), brandAPI.getAll(), unitAPI.getAll()])
  ...
  return (
    <div className="space-y-5 max-w-4xl">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold text-slate-800">Add Product Manually</h1>
          <p className="text-sm text-slate-500 mt-0.5">All fields are optional — fill in what you know</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
        ...
        <div className="bg-white rounded-xl p-5 shadow-sm border border-slate-100">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input {...register('barcode')} className={inp} placeholder="Scan or enter barcode" />
            </div>
            <div>
              <label className={lbl}>HSN Code</label>
              <input {...register('hsn_code')} className={inp} placeholder="e.g. 6109" />
            </div>
            <div>
              <label className={lbl}>Status</label>
              <select {...register('is_active')} className={inp}>
        ...
        <div className="bg-white rounded-xl p-5 shadow-sm border border-slate-100">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div>
              ...
              <select {...register('brand_id')} className={inp}>
            ...
            {[
              ...
              <div key={key}>
                ...
                <div className="relative">
                  ...
                  <input type="number" step="0.01" min="0" {...register(key)}
        ...
        <div className="bg-white rounded-xl p-5 shadow-sm border border-slate-100">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className={lbl}>Tax Rate (GST %)</label>
              <select {...register('tax_rate')} className={inp}>
                {TAX_RATES.map((r) => <option key={r} value={r}>{r}%</option>)}
              </select>
            </div>
            <div>
              <label className={lbl}>Tax Type</label>
              <select {...register('tax_type')} className={inp}>
        ...
        <div className="bg-white rounded-xl p-5 shadow-sm border border-slate-100">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
            {[
              ...
              <div key={key}>
                <label className={lbl}>{label}</label>
                <input type="number" min="0" {...register(key)} className={inp} placeholder="0" />
              </div>
            ))}
          </div>
        ...
        <div className="bg-white rounded-xl p-5 shadow-sm border border-slate-100">
          ...
          {imagePreview ? (
            ...
            <div {...getRootProps()}
              className={`border-2 border-dashed rounded-xl p-8 text-center cursor-pointer transition-colors ${isDragActive ? 'border-amber-400 bg-amber-50' : 'border-slate-200 hover:border-amber-300 hover:bg-slate-50'}`}>
              <input {...getInputProps()} />
