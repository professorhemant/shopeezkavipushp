  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Firms / Businesses</h1>
          <p className="text-sm text-gray-500 mt-0.5">Manage your business profiles</p>
        </div>
      ...
      {loading ? (
        <div className="flex items-center justify-center py-20"><div className="animate-spin w-6 h-6 border-2 border-blue-600 border-t-transparent rounded-full" /></div>
      ) : firms.length === 0 ? (
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 flex flex-col items-center justify-center py-20 text-gray-400">
          ...
          <p className="text-base font-medium text-gray-500">No firms configured</p>
          <p className="text-sm mt-1">Add your business details to get started</p>
          <button onClick={openAdd} className="mt-4 bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg text-sm font-medium">Add Your Business</button>
        </div>
      ) : (
        <div className="grid md:grid-cols-2 gap-4">
          {firms.map((firm) => (
            <div key={firm.id} className={`bg-white rounded-xl shadow-sm border-2 p-6 relative ${firm.id === activeFirmId ? 'border-blue-500' : 'border-gray-100'}`}>
              ...
              <div className="flex items-start gap-3 mb-4">
                ...
                <div className="flex-1 min-w-0">
                  <h3 className="font-bold text-gray-900 text-lg">{firm.name}</h3>
                  {firm.legal_name && firm.legal_name !== firm.name && <p className="text-sm text-gray-500">{firm.legal_name}</p>}
                </div>
              </div>
              <div className="space-y-1.5 text-sm">
                {firm.gstin && <div className="flex gap-2"><span className="text-gray-400 w-16 flex-shrink-0">GSTIN:</span><span className="font-mono text-gray-700">{firm.gstin}</span></div>}
                {firm.phone && <div className="flex gap-2"><span className="text-gray-400 w-16 flex-shrink-0">Phone:</span><span className="text-gray-700">{firm.phone}</span></div>}
                {firm.email && <div className="flex gap-2"><span className="text-gray-400 w-16 flex-shrink-0">Email:</span><span className="text-gray-700">{firm.email}</span></div>}
                {firm.address && <div className="flex gap-2"><span className="text-gray-400 w-16 flex-shrink-0">Address:</span><span className="text-gray-700">{[firm.address, firm.city, firm.state, firm.pincode].filter(Boolean).join(', ')}</span></div>}
              </div>
              <div className="flex gap-2 mt-4">
                <button onClick={() => openEdit(firm)} className="flex items-center gap-1.5 bg-gray-100 hover:bg-gray-200 text-gray-700 px-3 py-1.5 rounded-lg text-sm">
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4 overflow-y-auto">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-2xl my-4">
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
              <h2 className="text-lg font-semibold text-gray-900">{editing ? 'Edit Firm' : 'Add Firm'}</h2>
            ...
            <form onSubmit={handleSave} className="p-6 space-y-6">
              ...
              <div className="flex gap-3 pt-2">
                ...
                <button type="submit" disabled={saving} className="flex-1 bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg text-sm font-medium disabled:opacity-50">{saving ? 'Saving...' : 'Save Firm'}</button>
              </div>
            </form>
          </div>
        </div>
